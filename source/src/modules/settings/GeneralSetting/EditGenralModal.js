...
const EditGenralModal = ({
    ...
    const updateSetting = (values) => {
        const settingValue = {};
        if (data?.settingKey == "cod_service") {
            settingValue.min = values.min;
            settingValue.max = values.max;
            values.settingValue = JSON.stringify(settingValue);
        }
    ...
    useEffect(() => {
        if (data?.settingKey == 'cod_service') {
            const codService = JSON.parse(data.settingValue);
            form.setFieldsValue({
                ...data,
                ...codService,
            });
    ...
    const renderField = () => {
        ...
        else if (dataType == dataTypeSetting.OBJECT) {
            return (
                <>
                    <Col span={12}>
                        <NumericField
