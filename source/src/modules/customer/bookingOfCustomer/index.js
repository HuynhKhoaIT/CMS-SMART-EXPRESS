...
import { convertUtcToLocalTime, formatDateToEndOfDayTime, formatDateToZeroTime, formatMoney } from '@utils';
...
function BookingListPage() {
    ...
    const { execute: executeGet, loading: loadingDetail } = useFetch(apiConfig.booking.getById, {
        immediate: false,
    });
    ...
    const columns = [
...

        {
            ...
            render: (dataRow) => {
                return (
                    ...
                    >
                        <Tooltip placement="bottom" title={dataRow?.pickupAddress} color={'#000'}>
                            <div
                                style={{
                                    overflow: 'hidden',
                            ...
                            >
                                Điểm đón: {dataRow?.pickupAddress}
                            </div>
                        </Tooltip>
                        <Tooltip placement="bottom" title={dataRow?.destinationAddress} color={'#000'}>
                            <div
                                style={{
                                    overflow: 'hidden',
                                    textOverflow: 'ellipsis',
        ...
        mixinFuncs.renderActionColumn({ delete: true }, { width: '170px' }),
    ].filter(Boolean);
    ...
    const setBreadRoutes = () => {
        ...
        breadRoutes.push({ breadcrumbName: translate.formatMessage(message.booking) });
