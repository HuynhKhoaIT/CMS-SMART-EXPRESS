...
import { AppConstants, DEFAULT_FORMAT, DEFAULT_TABLE_ITEM_SIZE, WALLET_KIND_DRIVER } from '@constants';
...
const message = defineMessages({
    objectName: 'Ví tài xế',
});

function WalletListSum() {
    ...
    const columns = [
        ...
        {
            title: <FormattedMessage defaultMessage="Tổng tiền nhận" />,
        ...
        },
        {
            ...
            title: <FormattedMessage defaultMessage="Tổng tiền bồi thường" />,
            dataIndex: 'totalMoneyCompensate',
            width: 150,
            align: 'right',
            render: (totalMoneyCompensate) => {
                const formattedValue = formatMoney(totalMoneyCompensate, {
                    groupSeparator: ',',
                    decimalSeparator: '.',
                    currentcy: 'đ',
                    currentDecimal: '0',
                ...
                baseTable={
                    <BaseTable
                        ...
                        onRow={(record) => ({
                            onClick: (e) => {
                                e.stopPropagation();
                                navigate(routes.walletListPage.path + `?driverId=${record?.id}`);
                            },
                        })}
                    />
                }
